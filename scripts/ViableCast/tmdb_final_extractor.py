...
class TMDBFinalExtractor:
    ...
    def process_range(self):
        ...
        try:
            ...
            print(f"⏭️ Will skip rows that already have data in EITHER G (Episodes) OR H (Seasons)")
            print(f"🎯 Will ONLY process rows where BOTH G and H are completely empty")
            ...
            for row_num in range(self.start_row, self.end_row + 1):
                ...
                # Check if we already have ANY data in episodes OR seasons (skip if either has data)
                episodes_val = episodes.strip() if episodes else ""
                seasons_val = seasons.strip() if seasons else ""
                
                if episodes_val or seasons_val:
                    print(f"⏭️ TMDB: Row {row_num} - {cast_name} already has data (Episodes: '{episodes}', Seasons: '{seasons}') - SKIPPING")
