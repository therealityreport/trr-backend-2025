...
class FamousBirthdaysEnhancer:
    def __init__(self):
        """Initialize the Famous Birthdays scraper"""
    ...
    def search_famous_birthdays(self, cast_name, show_name):
        ...
        try:
            ...
            search_query = f"{cast_name} {show_name}".strip()
    ...
    def scrape_famous_birthdays_page(self, url, cast_name):
        ...
        try:
                ...
                if match:
                    try:
                        if len(match.groups()) == 3:
                            ...
                            print(f"   Debug - Birthday found: {result['birthday']}")
                            break
                    except Exception as e:
                            ...
                            if match:
                                try:
                                    if len(match.groups()) == 3:
                                        ...
                                        parsed_date = datetime.strptime(date_str, date_format)
                                        result['birthday'] = parsed_date.strftime('%Y-%m-%d')
                                        print(f"   Debug - Birthday parsed: {result['birthday']}")
                                        break
                                except Exception as e:
            ...
            # Extract bio - look for main bio text
            bio_elem = soup.find('div', class_='bio') or soup.find('p', class_='bio-text')
            if bio_elem:
                result['bio'] = bio_elem.text.strip()[:500]  # Limit to 500 chars
                print(f"   Debug - Bio length: {len(result['bio'])} chars")
            
            # Return result even if only partial data found
            has_data = any([result['birthday'], result['gender'], result['bio']])
            
            if has_data:
                print(f"   ✅ Extracted data - Birthday: {result['birthday']}, Gender: {result['gender']}, Bio: {'Yes' if result['bio'] else 'No'}")
                return result
            else:
    ...
    def update_spreadsheet(self, row_index, data, cast_name):
        ...
        try:
            ...
            # Update Gender column (column J = 10) if found
            if data.get('gender'):
                self.worksheet.update_cell(row_index, 10, data['gender'])
                updates_made.append(f"Gender: {data['gender']}")
                time.sleep(0.3)
            
            # Update Bio column (column K = 11) if found
            if data.get('bio'):
                self.worksheet.update_cell(row_index, 11, data['bio'])
                updates_made.append(f"Bio: {len(data['bio'])} chars")
                time.sleep(0.3)
    ...
    def process_range(self):
        ...
        try:
            ...
            for row_num in range(self.start_row, self.end_row + 1):
                ...
                gender = row[9] if len(row) > 9 else ''          # Column J: Gender
                bio = row[10] if len(row) > 10 else ''           # Column K: Bio
                
                # Skip if we already have all data
                if birthday and gender and bio:
                ...
                print(f"\n🎭 Row {row_num}/{self.end_row}: {cast_name} from {show_name or 'Unknown Show'}")
                
                # Search for the person
                fb_url = self.search_famous_birthdays(cast_name, show_name)
...
def main():
    """Main function"""
    print("🚀 Starting Famous Birthdays Data Enhancer...")
    ...
    enhancer = FamousBirthdaysEnhancer()
