...
CastInfo Input Structure (8 columns):
- Column A: CastName
- Column B: TMDb CastID (TMDb Person ID)
- Column C: Cast IMDbID  
- Column D: ShowName
- Column E: Show IMDbID
- Column F: TMDb ShowID
- Column G: TotalEpisodes
- Column H: TotalSeasons

UpdateInfo Output Structure:
- PersonName: Cast member's name
- PersonIMDbID: Their IMDb ID (from CastInfo Column C)
...
- ShowIMDbID: Comma-separated list of show IMDb IDs (from CastInfo Column E)
- ShowTMDbID: Comma-separated list of show TMDb IDs (from CastInfo Column F)
...
class UpdateInfoBuilder:
    ...
    def aggregate_cast_data(self, castinfo_data):
        ...
        for record in castinfo_data:
            # Extract data from CastInfo columns (using actual headers)
            cast_name = str(record.get('CastName', '')).strip()           # Column A
            cast_id = str(record.get('TMDb CastID', '')).strip()          # Column B (TMDb Person ID)
            cast_imdb_id = str(record.get('Cast IMDbID', '')).strip()     # Column C
            show_name = str(record.get('ShowName', '')).strip()           # Column D
            show_imdb_id = str(record.get('Show IMDbID', '')).strip()     # Column E
            show_id = str(record.get('TMDb ShowID', '')).strip()          # Column F (TMDb Show ID)
